...
do
  ...
  full=$(awk "{if (\$$i > 6 && \$$i <= 13) print 1}" "$filename" | wc -l)
  part=$(awk "{if (\$$i > 13 && \$$i <= 20) print 1}" "$filename" | wc -l)
  white=$(awk "{if (\$$i > 20 && \$$i <= 69) print 1}" "$filename" | wc -l)

  echo "$total" "$ue" "$white" "$full" "$part"
done
