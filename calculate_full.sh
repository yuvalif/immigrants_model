...
if [ "$#" -eq 1 ]; then
  prog=estimation_test_fullwage_$1
fi
...
for type in {0..2}
do
  average=$(awk -v type="$type" 'BEGIN{sum=0; count=0}{if ($3 == type && $(NF-1) > 13) {count=count+1; sum=sum+$NF}}END{print sum/count}' "$filename")

  rg1=$(awk -v type="$type" 'BEGIN{sum=0; count=0}{if ($3 == type && $(NF-1) > 13 && $(NF-1)%7 == 0) {count=count+1; sum=sum+$NF}}END{print sum/count}' "$filename")
  rg2=$(awk -v type="$type" 'BEGIN{sum=0; count=0}{if ($3 == type && $(NF-1) > 13 && $(NF-1)%7 == 1) {count=count+1; sum=sum+$NF}}END{print sum/count}' "$filename")
  rg3=$(awk -v type="$type" 'BEGIN{sum=0; count=0}{if ($3 == type && $(NF-1) > 13 && $(NF-1)%7 == 2) {count=count+1; sum=sum+$NF}}END{print sum/count}' "$filename")
  rg4=$(awk -v type="$type" 'BEGIN{sum=0; count=0}{if ($3 == type && $(NF-1) > 13 && $(NF-1)%7 == 3) {count=count+1; sum=sum+$NF}}END{print sum/count}' "$filename")
  rg5=$(awk -v type="$type" 'BEGIN{sum=0; count=0}{if ($3 == type && $(NF-1) > 13 && $(NF-1)%7 == 4) {count=count+1; sum=sum+$NF}}END{print sum/count}' "$filename")
  rg6=$(awk -v type="$type" 'BEGIN{sum=0; count=0}{if ($3 == type && $(NF-1) > 13 && $(NF-1)%7 == 5) {count=count+1; sum=sum+$NF}}END{print sum/count}' "$filename")
  rg7=$(awk -v type="$type" 'BEGIN{sum=0; count=0}{if ($3 == type && $(NF-1) > 13 && $(NF-1)%7 == 6) {count=count+1; sum=sum+$NF}}END{print sum/count}' "$filename")
  printf "%d\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\n" "$type" "$rg1" "$rg2" "$rg3" "$rg4" "$rg5" "$rg6" "$rg7" "$average"
done

printf "\n\nBlue Wage in Last Period\n"
